(function(){
  ...
  Renderer = function(canvas){
    ...
    var colors = ["red", "green", "blue", "yellow", "gray", "orange", "lime"];
      ...
      redraw:function(){
        ...
        particleSystem.eachEdge(function(edge, pt1, pt2){
          ...
          ctx.save()
            ...
            ctx.stroke()
            if(edge.data.message){
              ctx.fillStyle = "black";
              if(edge.data.message_color)
                ctx.fillStyle = edge.data.message_color;
              ctx.font = 'italic 13px sans-serif';
              ctx.fillText (edge.data.message, (pt1.x + pt2.x) / 2, (pt1.y + pt2.y) / 2);
            }
          ctx.restore()
        ...
        var handler = {
          ...
          clicked:function(e){
            ...
            if (nearest && selected && nearest.node===selected.node){
              var link = selected.node
              var i = Math.floor((Math.random() * 100)) % colors.length
              // sys.tweenNode(link.name, 0.1, {color:colors[i]})
              link.data.color= colors[i];
              return false
            }
          },
